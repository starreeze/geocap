...
However, the module is run by an LLM. So it is possible that some of the infos are not formatted into JSON file correctly. In this case, you can manually fix the result by using `eval_stage3.manual_fix_extraction` module to fix the extraction by reading logs in the eval result path (in which the result is saved). Since such an error is **not very likely** to happen (3/100k), it is considered necessary to exist but safe to ignore.
