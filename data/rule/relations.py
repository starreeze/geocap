...
class FusiformRelationGenerator:
    ...
    def generate_volutions_2(
        ...
        scale_factor = [normal(0.55, 0.03), uniform(1.1, 1.3)]
        x_delta = uniform(-0.05, -0.03)

        max_num_volutions = randint(4, 9)
        for i in range(max_num_volutions):
            self.fusiform = volutions[-1]

            x_delta = x_delta * (1.02**i)
            if "concentric" in volution_type:
    ...
    def get_random_fusiform(self, initial_chamber: Ellipse, fusiform_type: int) -> Fusiform | Fusiform_2:
        ...
        while volution_0 is None or not volution_0.is_closed():
            if fusiform_type == 1:
                focal_length = normal(1.2, 0.1)
                x_offset = 0.5
                y_offset = center[1] - normal(0.02, 2e-3)
                y_symmetric_axis = center[1]
                ...
                x_offset = center[0] - normal(1.8, 0.1) * initial_chamber.major_axis
                y_offset = center[1]
                power = max(3.0, normal(3.2, 0.1))
                x_symmetric_axis = center[0]
                epsilon = normal(1e-3, 1e-4)
                # epsilon = 0
                omega = 3 * np.pi
                phi = np.pi
                sin_params = [epsilon, omega, phi]
...
class SeptaGenerator:
    ...
    def generate_chomata(
        ...
        for i, volution in enumerate(volutions[:-step]):
            ...
            self.tunnel_angles_by_volution[i] = deepcopy(thetas)  # save for septa generation
    ...
    def generate_septa(
        ...
        for pole in poles_folds:
            angle_range = [pole["start_angle"], pole["end_angle"]]
            volution_range = [pole["start_volution"], pole["end_volution"]]
        ...
        for i, volution in enumerate(volutions[:-step]):
            ...
            for volution_range in poles_folds_volutions:
                if volution_range[0] <= i < volution_range[1]:
                    in_poles_folds_volution = True
            ...
            for t, theta in enumerate(thetas):
                # Ignore areas near the tunnel
                tunnel_angles = self.tunnel_angles_by_volution[i]
                if len(tunnel_angles) == 4:
                    tunnel_upper = tunnel_angles[:2]
                    tunnel_lower = tunnel_angles[2:]
                    if (
                        tunnel_upper[0] + 0.15 > theta > tunnel_upper[1] - 0.15
                        or tunnel_lower[0] + 0.15 > theta > tunnel_lower[1] - 0.15
                    ):
                        continue
                elif len(tunnel_angles) == 2:
                ...
                if in_poles_folds_angle and in_poles_folds_volution:
                    continue

                # Ignore angles near horizontal direction (0 <= theta <= 2pi)
                if 0.9 * np.pi < theta < 1.1 * np.pi or theta < 0.1 * np.pi or 1.9 * np.pi < theta:
                    continue
    ...
    def one_polygon_septa(
        ...
        if num_edges == 3:
            if size == "big":
                ...
                p3 = (center[0] + normal(0, 0.2 * interval), center[1] + normal(0, 0.2 * interval))
            points = [p1, p2, p3]
        elif num_edges == 4:
            if size == "big":
                p3 = next_p1
    ...
    def one_curve_septa(
        ...
        p2 = volution.get_point(theta - uniform(0.03, 0.05))
