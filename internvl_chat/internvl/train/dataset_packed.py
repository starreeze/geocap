...
class PackedDataset(IterableDataset):
    ...
    def split_buffer(buffer, max_tokens, img_start_token_id, img_token_id, img_end_token_id):
        ...
        logger.debug(
            f"split a sample into {len(splitted_buffer)} samples, " f"current max_tokens={max_tokens}"
        )
    ...
    def __iter__(self):
        ...
        logger.debug(
            f"{self.__class__.__name__} Rank {self.data_rank} " f"Worker {worker_id} begin to load data"
        )
        ...
        while True:
            ...
            while len(buffer_max_len_list) > 0:
                ...
                if (
                    self.strict_mode
                    and buffer_max_len_list[0]["pixel_values"].size(0) != self.num_images_expected
                ):
            ...
            while len(buffer_list) > self.max_buffer_size:
                ...
                if self.strict_mode:
                    yield self.postprocess_buffer(
                        self.pad_buffer(buffer_list.pop(0)), {"buffer_list": buffer_list}
                    )
                else:
                    yield self.postprocess_buffer(buffer_list.pop(0), {"buffer_list": buffer_list})
...
def packed_collate_fn(
    ...
    batch["statistics"] = torch.tensor(
        [num_samples, num_padding_tokens, batch["image_flags"].numel() - batch["image_flags"].sum().item()],
        dtype=torch.long,
    )
    batch.pop("type_ids")
    return batch
