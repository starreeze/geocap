    ...
    def __getitem__(self, idx):
        ...
        if self.few_shot > 0:
            ...
            for sample in few_shot_samples:
                ...
                few_shot_prompt += (
                    self.prompt.format(sample["image"], sample["question"]) + f" {sample['answer']}"
                )

        image = Image.open(image).convert("RGB")
        if self.dynamic_image_size:
            images = dynamic_preprocess(
                image, image_size=self.input_size, use_thumbnail=self.use_thumbnail, max_num=self.max_num
            )
        else:
            images = [image]
        ...
        for _, (pixel_values, questions, question_ids, annotations) in tqdm(enumerate(dataloader)):
            ...
            for question, question_id, answer, annotation in zip(
                ...
                if ds_name in [
                    ...
                    outputs.append({"question": question, "question_id": question_id, "answer": answer})
                elif ds_name in [
                    "docvqa_val",
                    "infographicsvqa_val",
                    "gqa_testdev",
                    ...
                    outputs.append({"question": question, "answer": answer, "annotation": annotation})
                elif ds_name in ["docvqa_test"]:
                    outputs.append({"questionId": question_id, "answer": answer})
                elif ds_name in ["vizwiz_test"]:
                    outputs.append({"image": question_id.replace("data/vizwiz/test/", ""), "answer": answer})
                else:
        ...
        if torch.distributed.get_rank() == 0:
            ...
            if ds_collections[ds_name]["metric"] == "vqa_score":
                ...
                if "gqa" in ds_name:
                    ...
                    print(
                        "python eval/vqa/convert_gqa_for_eval.py --src " + results_file + " --dst " + dst_file
                    )
                    python_path = "python"
                    os.system(
                        python_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datasets", type=str, default="okvqa_val,textvqa_val,vizwiz_val,ai2diagram_test,gqa_testdev_llava"
    )
    ...
    torch.distributed.init_process_group(
        backend="nccl", world_size=int(os.getenv("WORLD_SIZE", "1")), rank=int(os.getenv("RANK", "0"))
    )
