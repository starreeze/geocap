...
def validate_data(gtFilePath, submFilePath):
    ...
    for gtObject in gtJson["data"]:
        ...
        try:
            ...
            raise Exception(
                "The Det file is not valid. Question " + str(gtObject["questionId"]) + " not present"
            )
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    methodMetrics = {
        "score": (
            ...
            else totalScore / (len(gtJson["data"]) - len(question_ids_to_exclude))
        )
    }
    ...
    if show_scores_per_answer_type:
        for a_type, ref in answer_types.items():
            answer_types_scores[ref] = (
                0
                if len(gtJson["data"]) == 0
                else answerTypeTotalScore[a_type] / (answerTypeNumQuestions[a_type])
            )
        ...
        for e_type, ref in evidence_types.items():
            evidence_types_scores[ref] = (
                0
                if len(gtJson["data"]) == 0
                else evidenceTypeTotalScore[e_type] / (evidenceTypeNumQuestions[e_type])
            )
        ...
        for r_type, ref in reasoning_requirements.items():
            operation_types_scores[ref] = (
                0
                if len(gtJson["data"]) == 0
                else reasoningTypeTotalScore[r_type] / (reasoningTypeNumQuestions[r_type])
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--submission_file", type=str, help="Path of your method's results file.", required=True
    )
