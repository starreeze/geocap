...
ds_collections = {
    ...
    "MMMU_test": {"root": "MMMU/MMMU", "max_new_tokens": 10, "min_new_tokens": 1, "split": "test"},
    "MMMU_dev": {"root": "MMMU/MMMU", "max_new_tokens": 10, "min_new_tokens": 1, "split": "dev"},
}
...
class MMMUDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, root, split, prompt, input_size=224, dynamic_image_size=False, use_thumbnail=False, max_num=6
    ):
        # run for each subject
        sub_dataset_list = []
        for subject in tqdm(CAT_SHORT2LONG.values()):
            sub_dataset = load_dataset(
                root, subject, split=split, cache_dir=os.path.join(os.getcwd(), "data/MMMU/")
            )
            ...
            for idx, pil_image in enumerate(pil_images):
                if pil_image is not None:
                    if idx == 0:
                        ...
                        pil_image = dynamic_preprocess(
                            pil_image, image_size=self.input_size, use_thumbnail=self.use_thumbnail, max_num=1
                        )
                    images += pil_image
        else:
            images = [pil_images[0]]
    ...
    for ds_name in args.datasets:
        ...
        for _, (pixel_values, questions, answers, data_ids, options) in tqdm(enumerate(dataloader)):
            ...
            for question, pred, answer, data_id in zip(questions, preds, answers, data_ids):
                outputs.append(
                    {"question": question, "answer": pred, "gt_answers": answer, "data_id": data_id}
                )
        ...
        if torch.distributed.get_rank() == 0:
            ...
            if ds_collections[ds_name]["split"] == "validation":
                print("Evaluating ...")
                cmd = (
                    f"python eval/mmmu/main_eval_only.py "
