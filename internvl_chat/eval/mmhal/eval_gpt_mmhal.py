...
if __name__ == "__main__":
    ...
    for i, record in enumerate(records):
        ...
        input_text = template.format(
            image_content, record["question"], record["gt_answer"], record["model_answer"]
        )
        ...
        while response is None:
            try:
                response = openai_client.chat.completions.create(
                    model=args.gpt_model, messages=[{"role": "user", "content": input_text}], temperature=0.0
                )
    ...
    print(
        ...
        ",".join([str(round(sum(scores_each[i]) / len(scores_each[i]), 2)) for i in range(8)]),
        flush=True,
    )
