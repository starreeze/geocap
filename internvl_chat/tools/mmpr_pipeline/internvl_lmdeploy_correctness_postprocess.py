...
gt_path_map = {"m3cot": "M3CoT/train_pair_with_res.jsonl"}
...
def check_answer(answer_pred, answer_gt, mode):
    ...
    if "vqa_score" in mode:
        merged_outputs = [{"pred_answer": answer_pred, "gt_answers": [answer_gt] * 10}]
...
def build_neg_based_on_correctness(lines, mode):
    pos_id2item, pos_inconsistent_id2item, neg_id2item, neg_format_id2item = (
        _build_items_based_on_correctness(lines, mode=mode)
    )
    ...
    for key in pos_id2item:
        ...
        for item_pos in pos_id2item[key]:
            for item_neg in neg_id2item[key]:
                ...
                if (
                    not allow_entailment
                    and item_pos["answer_pred"].lower() in item_neg["answer_pred"].lower()
                ):
                    info["entail_skip"] += 1
                    continue
    ...
    if question_only:
        ...
        for item in items_set:
            try:
                ...
                image = item[0]
            items_list.append({"image": image, "question": item[1], "answer": item[2]})
...
def save_pairs(pairs, save_path):
    ...
    for pair in distinct_pairs:
        ...
        if (image, question, chosen, rejected, answer_gt) in rejected_meta_dict:
            filtered_pair["rejected_meta"] = rejected_meta_dict[
                (image, question, chosen, rejected, answer_gt)
            ]
...
def main(args):
    for filename in os.listdir(args.data_dir):
        ...
        pairs_vqa_correctness_rules_save_path = os.path.join(
            save_dir, "raw", f"{ds_name}_pairs_vqa_correctness_rules.jsonl"
        )
        pairs_vqa_correctness_rules_and_claims_save_path = os.path.join(
            save_dir, "raw", f"{ds_name}_pairs_vqa_correctness_rules_and_claims.jsonl"
        )
        pairs_vqa_format_rules_save_path = os.path.join(
            save_dir, "raw", f"{ds_name}_pairs_vqa_format_rules.jsonl"
        )
        ...
        pos_id2item, pos_inconsistent_id2item, neg_id2item, neg_format_id2item, all_incorrect_keys = (
            build_neg_based_on_correctness(lines, mode=mode)
        )
        ...
        save_pairs(
            build_pairs_based_on_pos_neg(
                pos_id2item=pos_id2item, neg_id2item=neg_id2item, allow_entailment=args.use_correctness_cache
            ),
            pairs_vqa_correctness_rules_save_path,
        )
        save_pairs(
            build_pairs_based_on_pos_neg(
                pos_id2item=pos_id2item,
                neg_id2item=neg_format_id2item,
                allow_entailment=args.use_correctness_cache,
            ),
