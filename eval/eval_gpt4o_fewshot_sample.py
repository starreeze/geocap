...
def main():
    ...
    generator = APIGenerator(model_id, max_tokens=8196, sys_prompt=sys_prompt, temperature=1.0)

    with open("dataset/stage3/no_vis/test.jsonl", "r") as f:
        with jsonlines.open(
            f"eval_data/outputs/{model_name}_{num_fewshot}shot_sample.jsonl", mode="a", flush=True
        ) as writer:
            lines = list(f)
