...
def find_extrema_points(points):
    ...
    if extrema_points.size == 0:  
        logger.warning(f"No extrema points found. Using midpoint instead.")
...
def conv_scoring(coeff, side, t3_threshold = 0.5):
    ...
    score = np.exp((np.abs(coeff[0]) - t3_threshold) * 10)\
    + coeff[1] * positive * 3
    return score
...
def get_angles_and_slope(path, debug=False):
    ...
    upper_interval, lower_interval = get_interval_points(
        contour, x_min, x_max, center_y, 
        np.vstack([existing_upper, existing_lower])
    )
