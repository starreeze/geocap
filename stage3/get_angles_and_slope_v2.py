...
def find_extrema_points(points):
    ...
    if extrema_points.size == 0:  
        logger.warning(f"No extrema points found. Using midpoint {sorted_points[len(sorted_points) // 2]} instead.")
...
def separate_slopes(slope):
    ...
    if len(regions) != 4:
        raise ValueError(f"Not enough regions.")
...
def conv_scoring(coeff, side, t3_threshold = 0.5):
    ...
    score = relu((np.abs(coeff[0]) - np.abs(coeff[1]) - t3_threshold) * 10)\
    + coeff[1] * positive * 3
    return score
...
def get_convexity(contour, extremes, extended_lower, extended_upper, sample_rate):
    ...
    try:
        convex_score = [conv_scoring(d2x_dy2[0], 'u'), conv_scoring(d2x_dy2[1], 'u'), conv_scoring(d2x_dy2[2], 'd'), conv_scoring(d2x_dy2[3], 'd')]# ↑concave, ↓convex
    except:
...
def get_angles_and_slope(path, debug=False):
    ...
    upper_mid_points, lower_mid_points = classify_points(contour, mid_x, n=contour.shape[0] // 3, center=center)
    ...
    if debug:
        ...
        plt.title(f"Left: {left_angle * 180 / 3.1416:.2f}° | Right: {right_angle * 180 / 3.1416:.2f}° | Upper: {upper_angle * 180 / 3.1416:.2f}° | Lower: {lower_angle * 180 / 3.1416:.2f}°")
        plt.savefig(f"dataset/DEBUG_TEST.png")
        plt.clf()

    return left_angle, right_angle, upper_angle, lower_angle, convex_score[0], convex_score[1], convex_score[2], convex_score[3]
...
def main():
    ...
    try:
        left_angle, right_angle, upper_angle, lower_angle,\
        lu_slope, ru_slope, ld_slope, rd_slope = get_angles_and_slope(path, debug=True)
    except:
        logger.error(f"{name} Failed")
        assert 0
    logger.info(f"{name} Angle Info \nLeft: {left_angle * 180 / 3.1416:.2f}° | Right: {right_angle * 180 / 3.1416:.2f}° | Upper: {upper_angle * 180 / 3.1416:.2f}° | Lower: {lower_angle * 180 / 3.1416:.2f}°")
