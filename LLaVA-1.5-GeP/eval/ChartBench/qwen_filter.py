...
def query_llm_for_value(samples, undone_keys):
    ...
    response_list = []
    while patience > 0:
        response_all = get_qwen_answer(question=query_user_message)
        response_list = response_all.split("\n")
        if len(response_list) == split_K:
            break
        patience -= 1
...
def eval_json_wrt_model(answer_list, split_K=10):
    ...
    for idx in tqdm(range(len(samples))):
        ...
        if samples[idx]["type"]["QA"] == "Acc+":
            continue
        if (
            samples[idx]["type"]["QA"] == "GPT-acc"
            and "gpt_filter" in samples[idx]["conversation"][-1].keys()
        ):
            continue
...
if __name__ == "__main__":
    eval_json_wrt_model("/ChartBench/Result/raw/llava-1.5-7b.jsonl")
