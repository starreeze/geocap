...
parser.add_argument(
    "--scenes", default="{tier}_sceneGraphs.json", type=str, help="Scene graphs file name format."
)
parser.add_argument(
    "--questions", default="{tier}_questions.json", type=str, help="Questions file name format."
)
parser.add_argument("--choices", default="{tier}_choices.json", type=str, help="Choices file name format.")
parser.add_argument(
    "--predictions", default="{tier}_predictions.json", type=str, help="Answers file name format."
)
parser.add_argument(
    "--attentions", default="{tier}_attentions.json", type=str, help="Attentions file name format."
)
parser.add_argument(
    "--consistency",
    action="store_true",
...
parser.add_argument(
    "--grounding", action="store_true", help="True to compute grounding score (If model uses attention)."
)
parser.add_argument(
    "--objectFeatures", action="store_true", help="True for object-based attention (False for spatial)."
)
parser.add_argument(
    "--mapSize",
    default=7,
...
questions = loadFile(args.questions.format(tier="./playground/data/eval/gqa/data/questions1.2/" + args.tier))
...
predictions = loadFile("./LLaVA-1.5-GeP/eval/result/gqa/data/testdev_balanced_predictions.json")
...
def computeGroundingScore(question, sceneGraph, attentionMap):
    ...
    if args.objectFeatures:
        ...
        cells = [
            (getCell(i, j), attentionMap[i][j]) for i in range(args.mapSize) for j in range(args.mapSize)
        ]
