...
class Generator:
    ...
    def load_llm_generator(self, model, system_prompt):
        """Initialize the LLM generator"""
        assert not self.loaded_llm
        # Initialize llm
        self.llm_generator = APIGenerator(model, max_tokens=2048, sys_prompt=system_prompt)
    ...
    def generate(self, pictures, questions, references) -> list[dict]:
        ...
        for p, q, r in zip(pictures, questions, references):
            entry = {
                "img": p,
                "question": q,
                "output": "", # later fill this
                "reference": r
            }
            data.append(entry)
        for idx, output in tqdm(enumerate(self.llm_generator([[("text", text),("image", pic)] for text, pic in zip(questions, pictures)])), total = len(pictures), desc="Generating Descriptions:"):
...
def main():
    ...
    generator = Generator()
    generator.load_llm_generator("gpt-4o", sys_prompt)
    pics, questions, references = [],[],[]
    with open("instructions.jsonl", "r") as f:
        for line in f:
            data = json.loads(line)
            pics.append(f"/home/nfs05/xiangch/geocap/dataset/common/filtered_images/{data['image']}") # No not here...
