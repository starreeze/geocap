...
class ModelArguments:
    ...
    tune_visual_encoder: bool = field(default=False)
    tune_mm_mlp_adapter: bool = field(default=False)
    tune_llm: bool = field(default=False)
    vision_tower: Optional[str] = field(default=None)
    mm_vision_select_layer: Optional[int] = field(default=-1)  # default to the last layer
    pretrain_mm_mlp_adapter: Optional[str] = field(default=None)
    pretrain_visual_encoder: Optional[str] = field(default=None)
...
def save_component(trainer: transformers.Trainer, output_dir: str, component: str, param_keys=None):
    keys_to_match = [component] if param_keys is None else param_keys
    if getattr(trainer.args, "use_im_start_end", False):
        keys_to_match.extend(["embed_tokens", "embed_in"])

    weight_to_save = get_component_maybe_zero_3(trainer.model.named_parameters(), keys_to_match)
    ...
    if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
        if current_folder.startswith("checkpoint-"):
            component = os.path.join(parent_folder, component)
            os.makedirs(component, exist_ok=True)
            torch.save(weight_to_save, os.path.join(component, f"{current_folder}.bin"))
        else:
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if getattr(trainer.args, "tune_llm", False):
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        save_component(trainer, output_dir, "mm_projector")

    if getattr(trainer.args, "tune_visual_encoder", False):
        save_component(trainer, output_dir, "vision_tower")
...
def preprocess_llama_3(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    # whitespace should come with later tokens: len('A B') = 2, len('A ') = 2, len(' B') = 1
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            has_bos_token = i == 0 and offset  # bos token will only appear in the first round
            if has_image:
                round_len = len(tokenizer_image_token(rou, tokenizer))
                instruction_len = len(tokenizer_image_token(parts[0], tokenizer)) - int(has_bos_token)
            else:
                round_len = len(tokenizer(rou).input_ids)
                instruction_len = len(tokenizer(parts[0]).input_ids) - int(has_bos_token)

            target[cur_len : cur_len + instruction_len] = IGNORE_INDEX

            cur_len += round_len + 1  # sep2 (eos)
...
def preprocess_phi_3(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
...
def train(attn_implementation=None):
    ...
    # open("test.txt", "a").write(str(model.model.get_vision_tower().vision_tower.vision_model.post_layernorm.weight.shape))
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.tune_mm_mlp_adapter != model_args.tune_llm:
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = model_args.tune_mm_mlp_adapter

        model.config.tune_visual_encoder = training_args.tune_visual_encoder = model_args.tune_visual_encoder
        if model_args.tune_visual_encoder != model_args.tune_llm:
            for p in model.get_model().vision_tower.parameters():
                p.requires_grad = model_args.tune_visual_encoder
