...
class LeafVeinDetector:
    ...
    def line_search(self, start_point, initial_direction, start_angle, end_angle):
        """
        ...
        start_angle: 搜索角度范围的起始角度
        end_angle: 搜索角度范围的终止角度
        ...
        for i in range(self.max_iterations):
            ...
            # 找到最佳搜索方向
            best_angle, best_score = self.find_best_direction_with_rotation(x, y, prev_angle, start_angle, end_angle)
    ...
    def search_branch_from_node(self, start_point, initial_direction, start_angle, end_angle):
        """
        从指定主脉节点开始搜索分支叶脉。
        """
        # 检查开始分支搜索条件
        # if not self.should_start_branch_search(start_point, initial_direction):
        #     return []
        branch_path = self.line_search(start_point, initial_direction, start_angle, end_angle)
        return branch_path
    ...
    def search_branches(self):
        ...
        for i, node in enumerate(search_points):
            # 左分支搜索
            left_branch = self.search_branch_from_node(node, initial_direction=135, start_angle=110, end_angle=150)
            ...
            # 右分支搜索
            right_branch = self.search_branch_from_node(node, initial_direction=45, start_angle=30, end_angle=70)
    ...
    def preprocess_image(self, image_path, mask_path="mask.npy", resize_factor=2):
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (erode_kernel_size * 2 + 1, erode_kernel_size * 2 + 1))
        ...
        binary_mask = cv2.adaptiveThreshold(
            gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, blockSize=39, C=5
        )
    ...
    def detect(
        self, image_path, start_point, initial_direction=90, output_path="vein_output.jpg"
    ):
        """
        从初始点开始检测叶脉并可视化。
    ...
    def visualize_results(self, output_path):
        ...
        if len(self.main_vein_path) > 1:
            ...
            for i, pt in enumerate(self.main_vein_path):
                if i == 0:
                    cv2.circle(binary_vis, pt, 4, (255, 255, 0), -1)
                elif i == len(self.main_vein_path) - 1:
                    cv2.circle(binary_vis, pt, 4, (255, 255, 0), -1)
                else:
        ...
        for branch in self.branches["left"]:
            if len(branch) > 1:
                for i in range(len(branch) - 1):
                    pt1 = branch[i]
                    pt2 = branch[i + 1]
                    cv2.line(binary_vis, pt1, pt2, (0, 255, 0), 2)
                cv2.circle(binary_vis, branch[0], 3, (0, 255, 255), -1)
        ...
        for branch in self.branches["right"]:
            if len(branch) > 1:
                for i in range(len(branch) - 1):
                    pt1 = branch[i]
                    pt2 = branch[i + 1]
                    cv2.line(binary_vis, pt1, pt2, (0, 0, 255), 2)
                cv2.circle(binary_vis, branch[0], 3, (0, 255, 255), -1)
    ...
    def print_statistics(self):
        ...
        total_branch_points = sum(len(b) for b in self.branches["left"]) + sum(len(b) for b in self.branches["right"])
...
def detect_leaf_veins_from_seed(image_path, start_point=(265 * 2, 1030 * 2), output_path="vein_output.jpg"):
    ...
    return detector.detect(
        image_path, start_point, output_path=output_path
    )
